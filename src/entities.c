...
void entity_handle_forces_and_impulses(struct entity* entity, float dt) {
...

    {
        ...
        /* damage has more priority */

        if (entity->apply_damage_knockback_force_timer > 0) {
            const int KNOCKDOWN_MAGNITUDE_X = 2;
            const int KNOCKDOWN_MAGNITUDE_Y = 2;

            switch (entity->damage_direction) {
                /* on left and right, cancel knockback if you're facing away.
                   I have no idea how accurate this looks, but this'll probably prevent a weird issue?*/
                case ATTACK_DIRECTION_RIGHT: {
                    if (entity->facing_dir == 1) {
                        ...
                        entity->apply_damage_knockback_force_timer = 0;   
                    }
                } break;
                case ATTACK_DIRECTION_LEFT: {
                    if (entity->facing_dir == -1) {
...
void entity_handle_dash(struct entity* entity, bool input) {
    if (input) {
...
void entity_handle_jump(struct entity* entity, bool input) {
    if (input) {
        if ((entity->onground ||
...
void entity_handle_basic_melee_attack(struct entity* entity, bool input, bool aiming_up, bool aiming_down) {
    if (input) {
...
void do_player_entity_input(struct entity* entity, int gamepad_id, float dt) {
    ...
    if (entity->movement_flags & MOVEMENT_FLAG_ALLOW_DASH) {
        entity_handle_dash(entity, dash);
    }
...
void entity_apply_attack_knockback(struct entity* entity, int direction, float length) {
    entity->attack_direction                   = direction;
    entity->apply_attack_knockback_force_timer = length;
}

void entity_apply_damage_knockback(struct entity* entity, int direction, float length) {
    entity->damage_direction                   = direction;
    entity->apply_damage_knockback_force_timer = length;
}

void hitbox_try_and_apply_knockback(struct hitbox* hitbox, struct entity* attacked, float time) {
